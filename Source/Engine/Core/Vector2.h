...
namespace MEN
{
	...
	public:
		...
		inline float DistanceSqr(const Vector2& vector) const { return (vector - *this).LengthSqr(); }
		inline float Distance(const Vector2& vector) const { return (vector - *this).Length(); }
	...
	inline std::istream& operator >> (std::istream& stream, Vector2& vector)
	{
		std::string line;
